...
# new_words1 = [p for p in products if len(p) > 6]
# # new_words1 = filter(lambda p: len(p) > 6, products)
# new_words2 = list(filter(lambda p: 'e' in p, products))
#
#
# new_words1 = [p for p in products if len(p) > 6] # [что откуда условие]
# new_dict = {i: p for i, p in enumerate(products) if len(p) > 6}
#
# print(new_dict)
#
# new_words3 = []
...
# products_sample = filter(lambda x: 'pa' in x.lower(), products)
...
# https://docs.python.org/3.10/library/typing.html#typing.Iterator
#
# https://geekbrainspro.notion.site/5-b0d593d717954923a45287a166680d5b
# my_str = 'https://geekbrainspro.notion.site/5-b0d593d717954923a45287a166680d5b'
# my_list = [1, 2, 4]
# print(my_list)
# def my_func(*args):
...
# DISCOUNT = 7
# prices = ['1578.4', '892.4', '354.1', '871.5']
# lst = list(map(lambda x: round(x * 0.93, 2), map(float, prices)))
# print(lst)
#
# result = list(map(lambda x: round((float(x)*DISCOUNT/100), 2), prices))
# print(result)
#
# prices_discount = list(map(lambda x: round(float(x)*0.07, 2), prices))
# print(prices_discount)
#
# pricesNew = list(map(lambda x: float(x)*(100-DISCOUNT)/100, prices))
# print(pricesNew)
#
# prices_discount = map(
